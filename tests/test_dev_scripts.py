...
def test_dev_sh_is_executable_and_demo_default():
    sh = ROOT / "dev.sh"
    assert sh.is_file()
    mode = sh.stat().st_mode
    assert mode & stat.S_IXUSR, "dev.sh must be chmod +x"
    body = sh.read_text()
    assert "C5_DEMO_MODE=1" in body
    assert "C5_TRADING_MODE=paper" in body
    assert "npm run dev:all" in body
    assert "http://localhost:5173" in body


def test_dev_ps1_exists_and_demo_default():
    ps1 = ROOT / "dev.ps1"
    assert ps1.is_file()
    body = ps1.read_text()
    assert 'C5_DEMO_MODE' in body and '"1"' in body
    assert "npm run dev:all" in body
    assert "http://localhost:5173" in body
    # must check for the prereqs explicitly so non-technical users get a
